...
var_est <- function(dat,gr_all, nparam_a,nparam_b,nparam_c,weights,sample_size,n.indiv,num.cluster,i){
  param_list<-list()
  nparam <- nparam_a + nparam_b + nparam_c
  for(p in 1:nparam){
    
    param_list[[p]] <- gr_all[(sample_size*p+1):(sample_size*(p+1))]
  }
  mat_gr <- data.frame(do.call(cbind,param_list),dat[[i]]$datares[[1]]$cluster,dat[[i]]$datares[[1]]$strata, dat[[i]]$datares[[1]]$weight)
...
adfun <- function(dat,n.iter,sample_size, nparam_a,nparam_b,nparam_c,nalpha,nbeta,ngamma, n.indiv, num.cluster, numbX, n.strata, type) { 
  ...
  for(i in 1:n.iter){
    ...
    if (i %% 20 == 0) cat("Iteration:" , i, "is done\n")
...
typevec <- c("UW","W","CAL")
typevec[1]
saveRDS(sim_cal, paste(typevec[2], ".rda", sep=""))
paste(typevec[2], ".rda", sep="")
```
...
xmat<-list()
      ...
      repsum <- summary(rep, "report") #estimated a's
      a_est<-repsum[1:sample_size]
      gr_a <- repsum[(sample_size+1):(sample_size*(nparam_a+1)] * weight
  param_list_a<-list()
  for(a in 1:nparam_a){
  ...
  invhessa <- solve(opt$hessian) #this is weighted Hessian
  se_a <- sqrt(diag(invhessa %*% G %*% invhessa))
```
```{r}
param_list_a[[5]]
