...
adfun <- function(dat,n.iter,sample_size, nparam_a,nparam_b,nparam_c,nalpha,nbeta,ngamma, n.indiv, num.cluster, numbX, n.strata, dfpop, type) { 
  ...
  for(i in 1:n.iter){
    ...
    if (type == "CAL"){
      ...
      colname_cost <- nX1col
      dfcost1 <- costmat<-as.matrix(dat[[i]]$phase1[,colname_cost , with = FALSE]) #dfcost
      ...
      la_fit<-tryCatch(suppressMessages(glmer(la_fml, family=binomial)),
                       error = function(e){
                         message(paste("An error occured for iteration (glmer-A)", i, ":\n"), e)})
     
      if(isTRUE(class(la_fit)=="NULL")) { next }
      ...
      ### change this to MakeADFun again
      #data <- list(Xrow=X_sample, N=sample_size, replicates=replicates, dfcost=dfcost1, weights=rep(1,2000),a1=a_est, b1=0, costindexsum=costindexsum, costsum=costsum, start=start)
...
scheme<-rep(c("INF"),each=18)
jobid_mat1<-cbind(sigmaA,sigmaB,p2,type,scheme)
jobid_mat <- jobid_mat1[-c(3,8,9),]
