...
sim_data <- function(sample_size, sigmaA.true, sigmaB.true){
  set.seed(3105)
  # Number of subjects
  N <- 10000 
  #sample_size <- 400 #parameter
  ...
  nalpha <- ncol(X); nbeta <- ncol(X);
  
  #for both binary and normal case?
  Z1 <- rbinom(N, 1, 0.4) #no repeated measure in surv mod.  Z1 is directly usable in cpp 
  Z2 <- rbinom(N, 1, 0.5) 
  Z3 <- rbinom(N, 1, 0.6)
  Z.1 <- Z1[idK]
  Z.2 <- Z2[idK]
  ...
  returnList <- list("df1"=df1, "maxrep"=maxrep,"nparam_a"=nparam_a, "nparam_b"=nparam_b, 
                     "nparam_c"=nparam_c,"numbaux"=numbaux,"numbX"=numbX)
  return(returnList)
}
...
nalpha <- sim_data_list$nalpha; nbeta<- sim_data_list$nbeta; ngamma <- sim_data_list$ngamma
numbaux<-sim_data_list$numbaux
numbX<-sim_data_list$numbX

#Sampling from population
n.strata <- 20 #each stratum has 10 clusters
...
var_est <- function(data,gr_all, nparam_a,nparam_b,nparam_c,weights,sample_size,n.indiv,num.cluster){
  param_list<-list()
  nparam <- nparam_a + nparam_b + nparam_c  # all params except for random effects (est)
  for(i in 1:nparam){
    
    param_list[[i]] <- gr_all[(sample_size*(i-1)+1):(sample_size*(i))]
  }
  mat_gr <- data.frame(do.call(cbind,param_list),data[[1]]$datares[[1]]$cluster,data[[1]]$datares[[1]]$strata, data[[1]]$datares[[1]]$weight)
  mat_gr1 <- mat_gr[1:(sample_size/2),]; mat_gr2 <- mat_gr[(sample_size/2+1):sample_size,]
  ...
  se <- sqrt(diag(invhess %*% G %*% invhess)) # returns a vector
  return(se)
}
...
adfun <- function(dat,n.iter,sample_size, nparam_a,nparam_b,nparam_c, nalpha,nbeta,ngamma,n.indiv, num.cluster, n.strata, type) { 
  ...
  # data passed to this is a list of data frames (1000)
  # extract gradient only when the type is weighted or calibrated
  #par <- matrix(NA,nrow=n.iter,ncol=nparam_a+nparam_b+nparam_c)
  ...
  for(i in 1:n.iter){
    ...
    if (type == "W"){
      ...
      repsum <- summary(rep, "report")
      #lc_gr <- repsum[(sample_size+1):(sample_size*nparam_c),]
      lc_gr <- repsum[1:sample_size*nparam_c]
      lc_gr <- lc_gr * weight # size different but weight recycled
    ...
    if (type == "CAL"){
      ...
      parameters <- list(sigma=c(0,1,rep(1,nalpha)))  
      ...
      rep<-sdreport(obj)
      repsum <- summary(rep, "report")
      lc_gr <- repsum[(sample_size*12+1):(sample_size*nparam_c)] #to extract gammas only
      #use this to calculate calibrated variance
      #exclude 10  hazs, cannot calibrate lambdas as randeffs are offsets
      
      #calibrated var est   :   may need to generalise this part 
      gr_gam1 <- lc_gr[1:sample_size] * cal_w_c;  #these are not weighed yet due to c++ code 
      gr_gam2 <- lc_gr[(sample_size+1):(2*sample_size)] * cal_w_c
      gr_gam3 <- lc_gr[(2*sample_size+1):(3*sample_size)] * cal_w_c
      ...
      score_gam1 <- resid(fit_gam1, type="response")  
