...
adfun <- function(dat,n.iter,sample_size, nparam_a,nparam_b,nparam_c,nalpha,nbeta,ngamma, n.indiv, num.cluster, numbX, n.strata, type) { 
  ...
  SE_cals <-matrix(NA,nrow=n.iter,ncol=ngamma)
  
  ncoldf <-ncol(dat[[1]]$datares[[1]]) #n. replicates same for all samples
  for(i in 1:n.iter){
    ...
    if (type == "UW"){
      ...
      obj <- MakeADFun(data,parameters,DLL="lb_general",silent=T)
      obj$hessian <- TRUE
      opt<-suppressWarnings(do.call("optim",obj))
      parest_b[i,] <- opt$par
    ...
    if (type == "W"){
      ...
      objb <- MakeADFun(data,parameters,DLL="lb_general",silent=T)
      objb$hessian <- TRUE
    ...
    if (type == "CAL"){
      ...
      fit_gam1 <- lm(gr_gam1~inf_fun[,1]) 
      fit_gam2 <- lm(gr_gam2~inf_fun[,2])
      fit_gam3 <- lm(gr_gam3~inf_fun[,3])
      
      score_gam1 <- cal_w_c * resid(fit_gam1, type="response")  
      score_gam2 <- cal_w_c * resid(fit_gam2, type="response")
      score_gam3 <- cal_w_c * resid(fit_gam3, type="response")
      ...
      SE_cals[i,] <- sqrt(diag(invhess %*% G %*% invhess))
    }
...
#running the simulations over different values of random effects and sample sizes for each type

# pop data to be sampled and some objects required in the later stage
sim_data_list<-sim_data(1,1) #population data 
...
sampledata<-lapply(1:n.iter, function(v) samplingfunc(v,df1,n.per.cluster=n.per.cluster))
...
saveRDS(sim_cal, paste(typevec[jobid], ".rda", sep=""))