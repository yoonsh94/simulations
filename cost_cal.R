...
sim_data <- function(N){
  ...
  X2 <- rnorm(Ntotal, X1, 1.5)#0.4
  X3 <- rnorm(Ntotal, X1, 0.8)#0.3 #X1 is now correlated with X2 and X3
  #X <- cbind(1,X1,X2,X3)
  
  #for both binary and normal case?
  Z1 <- rbinom(N, 1, 0.5) #no repeated measure for Z.  Z1 is directly usable in cpp 
  Z2 <- rnorm(N, Z1, 1.5) #0.9
  Z3 <- rnorm(N, Z1, 0.8)
  Z.1 <- Z1[idK]
...
samplingfunc <- function(iter,N,data,replicates,X, Z_one, Z, Z_aux,sigmaA.true,sigmaB.true, n.per.cluster){
  ...
  auxmat<-list()
  for(a in 1:3){
    assign(paste0("colname_aux", a), (1:ncoldf)[colnames(data.final) %in% paste0('aux', a, "_",1:maxrep)])
    assign(paste0("aux",a,"row"),as.matrix(data.final[,get(paste0("colname_aux",a)) , with = FALSE]) )
    auxmat[[a]] <- as.vector(t(get(paste0("aux",a,"row"))))
  }
  
  auxmat<-do.call("cbind",auxmat)
  aux_sample<-cbind(auxmat[!rowSums(!is.finite(auxmat)),])
  
  
  
  list(phase1=df1,datares=datares,X_sample=X_sample,aux_sample=aux_sample)
...
adfun <- function(dat,n.iter,sample_size, nparam_a,nparam_b,nparam_c,nalpha,nbeta,ngamma, n.indiv, num.cluster, numbX, n.strata, dfpop, type) { 
  ...
  #la_fml_offset <- as.formula(paste("YNoZero_sample~",paste(xn,collapse="+")))
                 ...
                 la_imp_offset<-glm(YNoZero_sample ~ X_phase1[, 1] + X_phase1[, 2] + X_phase1[, 3] + 
                                      X_phase1[, 4] + offset(a_est[idK_sample]), family=binomial) #fitting standard glm using estimated a's as offsets,  now this model  is used to get IFs
                 ...
                 lb_imp_offset <- glm(cost2_nozero ~ X_sample_nozero[, 1] + X_sample_nozero[, 2] + 
...
type<-rep(c("UW","W","CAL-C"),each=6)
jobid_mat1<-cbind(sigmaA,sigmaB,p2,type)
jobid_mat <- jobid_mat1[c(2,3),]
...
n.iter<- 10 # make this 500 or 1000
